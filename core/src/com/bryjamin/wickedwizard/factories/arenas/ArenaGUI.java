    ...
    public void drawArenaDoors(SpriteBatch batch, Arena arena){
        for(com.bryjamin.wickedwizard.ecs.components.object.DoorComponent dc : arena.doors) {
            ...
            if((diffX < range && diffX > -range) &&  (diffY < range && diffY > -range)) {
                ...
                //Left Line
                if (dc.exit == com.bryjamin.wickedwizard.utils.enums.Direction.LEFT) {
                    drawLeftLine(batch, x, y + MINI_SIZE, LINE_THICKNESS, SIZE / 2);
                }

                //Right Line
                if (dc.exit == com.bryjamin.wickedwizard.utils.enums.Direction.RIGHT) {
                    drawRightLine(batch, x + SIZE, y + MINI_SIZE, LINE_THICKNESS, SIZE / 2);
                }

                //Top Line
                if (dc.exit == com.bryjamin.wickedwizard.utils.enums.Direction.UP) {
                    drawTopLine(batch, x + MINI_SIZE, y + SIZE, SIZE / 2, LINE_THICKNESS);
                }

                //Bottom Line
                if (dc.exit == com.bryjamin.wickedwizard.utils.enums.Direction.DOWN) {
